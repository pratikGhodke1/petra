...
if len(st.session_state.messages) == 0 and not st.session_state.called_agent:
    ...
    render_suggestions(
        "Suggest 4 starting questions that user's may like to ask strictly based on knowledge base content related different products, documents or services mentioned in the knowledge base. Make sure these questions can be answered with 100% confident and no ambiguity when sent again."
    )
...
if st.session_state.messages and st.session_state.messages[-1]["role"] == "assistant":
    ...
    render_suggestions(
        "Suggest 2 short follow-up prompt recommendations relevant to chat history and 2 other suggestions based on knowledge base. Make sure these questions can be answered with 100% confident and no ambiguity when sent again."
    )
